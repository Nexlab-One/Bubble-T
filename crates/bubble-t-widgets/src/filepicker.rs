...
//! It integrates seamlessly with bubble-t applications and supports customizable
...
//!
//! ```rust
//! use bubble_t_widgets::filepicker::{Model, Styles, FilepickerKeyMap};
...
///
/// ```rust
/// use bubble_t_widgets::filepicker::ErrorMsg;
///
...
///
/// ```rust
/// use bubble_t_widgets::filepicker::{ReadDirMsg, FileEntry};
...
///
/// ```rust
/// use bubble_t_widgets::filepicker::FilepickerKeyMap;
/// use bubble_t_widgets::key::KeyMap;
///
...
///
/// ```rust
/// use bubble_t_widgets::filepicker::Styles;
/// use lipgloss_extras::prelude::*;
///
...
///
/// ```rust
/// use bubble_t_widgets::filepicker::FileEntry;
/// use std::path::PathBuf;
///
/// let entry = FileEntry {
...
/// the BubbleTeaModel trait for integration with bubble-t applications.
///
/// # Examples
///
/// ```rust
...
impl Model {
    ...
    pub fn read_dir_cmd(&self) -> Cmd {
        // Use bubble-t tick with minimal delay to create an immediate command
        let current_dir = self.current_directory.clone();
        let id = self.id;

        bubble_t::tick(std::time::Duration::from_nanos(1), move |_| {
...
///
/// ```rust
/// use bubble_t_widgets::filepicker::is_hidden_name;
///
...
impl BubbleTeaModel for Model {
    ...
    fn update(&mut self, msg: Msg) -> Option<Cmd> {
        ...
        if let Some(_window_msg) = msg.downcast_ref::<bubble_t::WindowSizeMsg>() {
            if self.auto_height {
    ...
    fn view(&self) -> String {
        ...
        for (i, f) in self.files.iter().enumerate() {
            ...
            if self.selected == i {
                ...
                if f.is_symlink
                    && let Some(target) = &f.symlink_target
                {
