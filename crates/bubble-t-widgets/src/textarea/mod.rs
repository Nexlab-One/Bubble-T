...
//! ### Example
//! ```rust
//! use bubble_t_widgets::{textarea, Component};
//!
...
use crate::{Component, cursor, viewport};
use bubble_t::{Cmd, Model as BubbleTeaModel};
...
impl Model {
    ...
    fn merge_line_below(&mut self, row: usize) {
        ...
        if let Some(next_line) = self.value.get(row + 1).cloned()
            && let Some(current_line) = self.value.get_mut(row)
        {
    ...
    fn merge_line_above(&mut self, row: usize) {
        ...
        if let Some(current_line) = self.value.get(row).cloned()
            && let Some(prev_line) = self.value.get_mut(row - 1)
        {
    ...
    pub fn view(&mut self) -> String {
        ...
        for (doc_line_idx, line) in self.value.iter().enumerate() {
            ...
            for (wrap_idx, wrapped_line) in wrapped_lines.iter().enumerate() {
                ...
                // Add padding
                s.push_str(&style.computed_text().render(&" ".repeat(padding)));
                s.push('\n');
            }
        }
    ...
    pub fn update(&mut self, msg: Option<bubble_t::Msg>) -> Option<bubble_t::Cmd> {
        if !self.focus {
            return None;
        }
        ...
        if let Some(msg) = msg {
            ...
            // Handle key messages
            if let Some(key_msg) = msg.downcast_ref::<bubble_t::KeyMsg>() {
    ...
    fn handle_key_msg(&mut self, key_msg: &bubble_t::KeyMsg) -> Option<bubble_t::Cmd> {
    ...
    fn handle_clipboard_keys(&mut self, key_msg: &bubble_t::KeyMsg) -> Option<bubble_t::Cmd> {
    ...
    fn handle_movement_keys(&mut self, key_msg: &bubble_t::KeyMsg) {
        use crate::key::matches_binding;
    ...
    fn handle_deletion_keys(&mut self, key_msg: &bubble_t::KeyMsg) {
        use crate::key::matches_binding;
    ...
    fn handle_text_operations(&mut self, key_msg: &bubble_t::KeyMsg) {
        use crate::key::matches_binding;
    ...
    fn handle_text_insertion(&mut self, key_msg: &bubble_t::KeyMsg) {
        use crate::key::matches_binding;
    ...
    fn handle_character_input(&mut self, key_msg: &bubble_t::KeyMsg) {
        // Handle regular character input
    ...
    fn extract_character_from_key_msg(&self, key_msg: &bubble_t::KeyMsg) -> Option<char> {
    ...
    fn paste_command(&self) -> bubble_t::Cmd {
        // Use tick with minimal duration to trigger clipboard read
        bubble_t::tick(
            std::time::Duration::from_millis(1),
            |_| match Self::read_clipboard() {
                Ok(content) => Box::new(PasteMsg(content)) as bubble_t::Msg,
                Err(err) => Box::new(PasteErrMsg(err)) as bubble_t::Msg,
            },
        )
    }
