//! Error types for bubble-t.
//!
//! This module defines the custom error types used throughout the `bubble-t` library.
...
//!
//! The `bubble-t` library follows Rust's idiomatic error handling patterns. All
...
//!
//! ```no_run
//! use bubble_t::{Program, Model, Msg, Error, Cmd};
//!
//! # struct MyModel;
...
/// The main error type for `bubble-t` operations.
///
...
///
/// ```no_run
/// use bubble_t::{Error, Program, Model, Msg, Cmd};
///
/// # struct MyModel;
/// # impl Model for MyModel {
/// #     fn init() -> (Self, Option<Cmd>) { (MyModel, None) }
